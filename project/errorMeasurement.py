...
class errorMeasurement:    
    def __init__(self):
        self.actual_value = 0
        self.pred_value = 0           
    def calculateErrorMeasure(self, actual, predicted, errorMeasures):
        ar = []
        mre = []
        mer = 0
        bre = 0
        ...
        errorMeasures.mar.append(float(sum(ar)) / max(len(ar), 1))
        errorMeasures.mmre.append(float(sum(mre)) / max(len(ar), 1))
        errorMeasures.mdmre.append(numpy.median(mre))
